...
int main()
{
    ...
    do {
        ...
        // If user chooses 1, we get ask how many seeds the user wants to have (make sure to reiterate what
        // type of seed they chose)
        // Use a loop to guarantee that the user enters a number between 1 and 21
        if(choice == 1)
        {
            chooseSeedType(currentSeedType);
            do{
            cout << "How many " << currentSeedType << " seeds would you like to plant?" << endl;
            cin >> numSeeds;
            if(numSeeds > 21 || numSeeds < 1)
